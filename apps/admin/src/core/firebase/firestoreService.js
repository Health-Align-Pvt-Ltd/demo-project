...
export const firestoreService = {
  ...
  async getProductsPaginated(lastDoc = null, pageSize = 20, action = 'all') {
    try {
      let baseQuery = collection(db, 'products');
      
      // Apply filtering based on action
      if (action === 'pending-records') {
        // For pending records, we might want to show products that need review
        // Since products don't have a status field, we'll need to determine what constitutes "pending"
        // For now, we'll just fetch all products as we don't have a clear definition of "pending" for products
      } else if (action === 'approved-records') {
        // For approved records, we show all products as they are considered approved by default
        // since there's no explicit approval process defined for products
      }
  ...
  async getProductCategoriesPaginated(lastDoc = null, pageSize = 20, action = 'all') {
    try {
      let baseQuery = collection(db, 'productCategories');
      
      // Apply filtering based on action
      if (action === 'pending-records') {
        // For pending records, we might want to show categories that need review
        // Since categories don't have a status field, we'll just fetch all categories
      } else if (action === 'approved-records') {
        // For approved records, we show all categories as they are considered approved by default
      }
  ...
  async getDiseasesPaginated(lastDoc = null, pageSize = 20, action = 'all') {
    try {
      let baseQuery = collection(db, 'diseases');
      
      // Apply filtering based on action
      if (action === 'pending-records') {
        // For pending records, we might want to show diseases that need review
        // Since diseases don't have a status field, we'll just fetch all diseases
      } else if (action === 'approved-records') {
        // For approved records, we show all diseases as they are considered approved by default
      }
  ...
  async getPharmaciesPaginated(lastDoc = null, pageSize = 20, action = 'all') {
    try {
      let baseQuery = collection(db, 'pharmacy');
      
      // Apply filtering based on action
      if (action === 'pending-records') {
        // For pending records, we might want to show pharmacies that need review
        // Since pharmacies don't have a status field, we'll just fetch all pharmacies
      } else if (action === 'approved-records') {
        // For approved records, we show all pharmacies as they are considered approved by default
      }
  ...
  async getAmbulancesPaginated(lastDoc = null, pageSize = 20, action = 'all') {
    try {
      let baseQuery = collection(db, 'ambulance');
      
      // Apply filtering based on action
      if (action === 'pending-records') {
        // For pending records, we might want to show ambulances that need review
        // Since ambulances don't have a status field, we'll just fetch all ambulances
      } else if (action === 'approved-records') {
        // For approved records, we show all ambulances as they are considered approved by default
      }
  ...
  async readPaginated(collectionName, lastDoc = null, pageSize = 20, orderByField = 'createdAt', action = 'all') {
    try {
      let baseQuery = collection(db, collectionName);
      
      // Apply filtering based on action if needed
      if (action === 'pending-records') {
        // For pending records, we might want to show items that need review
        // Since generic collections don't have a status field, we'll just fetch all items
      } else if (action === 'approved-records') {
        // For approved records, we show all items as they are considered approved by default
      }
